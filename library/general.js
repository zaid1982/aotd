...
function getBarCode(printNo, barcodeNo) {
    JsBarcode("#barcode", barcodeNo, {format: "CODE128", displayValue: false, height: 125, width: 6});
    $('#sampleNo').html(printNo);
...
function dateString2Date(dateString) {
    ...
    //alert(dt.slice(0,3).reverse().join('-') + ' ' + dt[3]);
...
function get_option(field, status, tablename, id_name, desc_name, status_name, defaults, sorts, extra_name, extra_value, mode_disabled) {
    if (typeof defaults === 'undefined')
        $('#'+field).html('<option value="" selected>-</option>');
    else if (typeof defaults !== 'undefined' && defaults !== null){
    ...
    $.ajax({
        ...
        success: function(resp){
            if (resp.success == true){ 
                ...
                if (typeof mode_disabled !== 'undefined' && mode_disabled == '1')
...
function f_load_profile(profile_id, form_tag, flags) {
    $.ajax({
        url: "process/p_referance.php",
        type: "POST",
        dataType : "json",
        ...
        success: function(resp){
            if (resp.success == true){ 
                $.each(resp.result, function(name, vals){
                    form_tag.substr(0,1) == 'l' ? $('#'+form_tag+'_'+name).html(replaceNull(vals, '-')) : $('#'+form_tag+'_'+name).val(vals);
                    if (name == 'address_id' && flags == 1)
                        f_load_address(vals, form_tag+'a');                    
                });
            } else {
                $('#lbl_modal_error').html('Kesilapan pada data. Sila hubungi sistem admin!');
                $('#modal_error').modal('show');
...
function chart_pie_3d(chart_div,title,subtitle,dataset) {
...
function chart_donut_3d(chart_div,title,subtitle,dataset) {
    Highcharts.chart(chart_div, {
        chart: {
            type: 'pie',
            options3d: {
...
function chart_complience(chart_div, title, subtitle, dataset, limitValue, units) {
    Highcharts.chart(chart_div, {
        chart: {
            type: 'spline',
...
function f_steps (arr_steps, current_turn, step_id) {
    var html = '';
...
function f_check_task_user (wfTaskType_id) {
    var task_user = f_get_general_info('wf_task_user', {user_id:$('#user_id').val(), wfTaskType_id:wfTaskType_id});
    if (!task_user)
        f_notify(2, 'Error', 'You are not set to perform this checkpoint task.');
    return (task_user);            
}
    
$('.modal').on('shown.bs.modal', function () {
    if($(this).attr('id') != 'modal_waiting' && $(this).attr('id') != 'modal_ats_formula_set' && $(this).attr('id') != 'modal_ats_raw' && $(this).attr('id') != 'modal_eff_field'
    && $(this).attr('id') != 'modal_upload' && $(this).attr('id') != 'modal_view_document') {
