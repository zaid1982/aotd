...
try {     
    /* Validate the form on the server side - 5100 */
    if (empty($_POST['funct'])) { // Function empty
        ...
        if ($_POST['funct'] == 'login') {     /* Login */            
            ...
            log_debug(__LINE__, 'Password = '.md5($_POST['txt_password']), $log_dir);
            if (empty($user_basic)) {
                throw new Exception('(ErrCode:5101) ['.__LINE__.'] - Login attempt fail', 30);
            } else if ($user_basic['user_password'] == md5($_POST['txt_password'])) {
                ...
                $profile = Class_db::getInstance()->db_select_single('profile', array('profile_id'=>$user_basic['profile_id']), '', 1);
                
                $_SESSION["login_name"] = $profile['profile_name'].' '.$profile['profile_lastname'];
            ...
            $wfGroup_id = Class_db::getInstance()->db_insert('wf_group', array('wfGroup_name'=>$_POST['mrg_company_name'], 'wfGroup_type'=>$wfGroup_type, 'wfGroup_regNo'=>$wfGroup_regNo, 'wfGroup_createdBy'=>$user_id));
            if ($_POST['mrg_jenis_permohonan'] == '1') {
                ...
                $fn_user->register_new_industry($user_id, $wfGroup_id, $_POST['mrg_doeFile_no']);
            }   
            ...
            //$fn_email->send_email_activation($_POST['mrg_profile_email'], $_POST['mrg_profile_name'], $_POST['mrg_profile_icNo'], $activation_key);
            $result = '1';
            ...
            if (!empty($arr_wfTaskType_id))
                $arr_notify_task = Class_db::getInstance()->db_select('dt_notify_task', array('wfGroup_id'=>$arrayParam['wfGroup_id']), NULL, NULL, 0, array('wfTaskType_id'=>implode(',', $arr_wfTaskType_id)));
