...
try {
    ...
    if (empty($_POST['funct'])) { // Function empty
        ...
        if (in_array($_POST['funct'], array('email_assign', 'email_process', 'email_verify', 'email_approval', 'email_revise', 'email_verify_initRATA', 'email_failReport_verify', 'email_certRenewal_verify'), true)) {
            ...
            try {
                ...
                foreach ($arr_user as $users) {
                    ...
                    if (empty($profile['profile_email']))
                        ...
                        else if ($_POST['funct'] == 'email_process')
        ...
        } else if (in_array($_POST['funct'], array('email_reject', 'email_return', 'email_approve', 'email_initialRATA', 'email_return_initRATA', 'email_failReport_user', 'email_certRenewal_result'), true)) {      
            $wf_task = Class_db::getInstance()->db_select_single('wf_task', array('wfTask_id'=>$arrayParam['wfTask_id']), NULL, 1);
            $wfFlow_id = Class_db::getInstance()->db_select_col('wf_task_type', array('wfTaskType_id'=>$wf_task['wfTaskType_id']), 'wfFlow_id', NULL, 1);
            $app_type = Class_db::getInstance()->db_select_col('wf_flow', array('wfFlow_id'=>$wfFlow_id), 'wfFlow_desc', NULL, 1);
            $wf_transaction = Class_db::getInstance()->db_select_single('wf_transaction', array('wfTrans_id'=>$wf_task['wfTrans_id']), NULL, 1);
            ...
            if ($_POST['funct'] == 'email_activation') {
                $fn_email->send_email_activation($profile['profile_email'], $profile['profile_name'], $user['user_name'], $user['old_password'], $user['user_activationKey']);
            } else if ($_POST['funct'] == 'email_user_creation') {            
                $role_list = Class_db::getInstance()->db_select_col('vw_user_types', array('user_id'=>$arrayParam['user_id']), 'role_list');
                $fn_email->send_email_user_creation($profile['profile_email'], $profile['profile_name'], $user['user_name'], $user['old_password'], $role_list);
