...
class Class_upload {
    ...
    public function upload_file ($type, $files=array(), $title='', $docName_id='', $remark='', $wfTrans_id='', $ids='') {
        try {
                    ...
                    if ($type == 1) {
                        ...
                        if ($userfile_extn == 'xls' || $userfile_extn == 'xlsx' || $userfile_extn == 'jpg' || $userfile_extn == 'pdf' || $userfile_extn == 'xls' || $userfile_extn == 'doc' 
                            ...
                            $document_id = Class_db::getInstance()->db_insert('document', array('document_name'=>$title, 'documentName_id'=>$docName_id, 'document_extension'=>$userfile_extn, 'document_timeUpload'=>'Now()', 'document_remarks'=>$remark, 'document_uplname'=>$userfile_name,
                                'wfTrans_id'=>$wfTrans_id, 'document_sampleCode'=>$ids, 'document_uploadBy'=>$_SESSION['user_id']));
                            ...
                            throw new Exception('(ErrCode:1104) ['.__LINE__.'] - Document type is not allowed to be uploaded = '.$userfile_extn, 31);
                    } else {
                        throw new Exception('(ErrCode:1102) ['.__LINE__.'] - Upload type error.');
                    }   
